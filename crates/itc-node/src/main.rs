...
//!
//! Slice 5 (this commit): block-body download + serve. After syncing headers the
//! node downloads full block bodies (`getdata MSG_BLOCK`), verifies each against
//! its header hash, persists them in NEDB, and serves block bodies to inbound
//! peers. Combined with header sync + seeding, it now sync-from / seed-to the
//! full chain (headers + bodies).
//!
//! Usage: `itc-node [LISTEN_PORT]`
//!   env `ITC_NODE_DATADIR`    — store path (default ./itc-node-data)
//!   env `ITC_NODE_MAX_BLOCKS` — block bodies to download this run (default 2016;
//!                               set 0 for unlimited)
...
fn main() {
    ...
    let max_blocks: usize = std::env::var("ITC_NODE_MAX_BLOCKS")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(2016);
    ...
    // ── 0. Open the NEDB store and resume any persisted chain ─────────────────
    ...
    // ── 2. Forward header sync (persists into NEDB) ───────────────────────────
    println!(
    ...
    // ── 3. Download block bodies (verify + persist) ───────────────────────────
    let cap = if max_blocks == 0 { "unlimited".to_string() } else { max_blocks.to_string() };
    println!("itc-node: downloading block bodies (max {cap}) ...");
    match blocks::download_blocks(&mut peer, &chain, &store, max_blocks) {
        Ok(n) => println!("itc-node: downloaded {n} new block bodies (engine head {})", store.head()),
        Err(e) => eprintln!("itc-node: block download error: {e}"),
    }

    // ── 4. Seed headers + block bodies to inbound peers ───────────────────────
